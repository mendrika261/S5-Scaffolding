...
public abstract class Scaffolding {
    ...
    public Scaffolding(HashMap<String, String> params,
        ...
        //setPackageName(Format.joinPackageName(packageName, params.getOrDefault("default_package", "")));
    ...
    protected void processDeps(String packageName, String name) { // eg: entity_package, #ClassName#
    ...
    private String processPackage() {
        return ((getPackageName() != null && !getPackageName().isEmpty()) || getParams().containsKey("default_package"))
                ? getLanguageParams().getOrDefault("#package#", "")
                    .replace("#packageName#",
                        Format.joinPackageName(getPackageName(),
    ...
    private String processImports() {
        ...
        for (String importName : getImports()) {
            importCode.append(getLanguageParams().get("#import#").replace("#importName#", importName));
        }
    ...
    private String processPrimaryKeyType() {
        ...
        if(mapping==null) return getTable().getPrimaryKey().getType();
    ...
    public void generate(String path) {
        String code = File.getFileContentAsString(getTemplatePath() + getParams().get("template"));
        code = evaluate(process(code));

        File.writeToFile(path +
                Format.toPathNameWithTrail(process(getParams().getOrDefault("default_package", ""))) +
                getParams().get("file_name")
                        .replace("#Name#", Format.toCamelCase(getTable().getName(), true))
                        .replace("#name#", Format.toCamelCase(getTable().getName(), false))
    ...
    public Mapping getMapping(String type) {
        final Mapping mapping = getLanguageMappings().getOrDefault(getStandardType(type),
                getLanguageMappings().get("default"));
        if(mapping==null)
            return new Mapping();
        return mapping;
    }
    ...
    public String getStandardType(String type) {
        return getProvider().getMappings().getOrDefault(type, getProvider().getMappings().get("default"));
    }

    public void addImport(String importNames) {
        if(importNames==null || importNames.isEmpty()) return;
    ...
    public HashMap<String, String> getLanguageParams() {
