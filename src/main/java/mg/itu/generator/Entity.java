...
public class Entity extends Scaffolding {
    public Entity(HashMap<String, String> params, HashMap<String, String> languageParams, HashMap<String, Mapping> languageMappings, String packageName, Table table, Provider provider, Database database) {
    ...
    protected String processAttributes() {
        ...
        for (Column column : getTable().getColumns()) {
            ...
            if(column.isPrimaryKey()) {
                ...
                addImport(getParams().getOrDefault("attribute_import_annotation", null));
            }
            attributePart += getLanguageParams().get("#attribute#");
            attributePart = attributePart.replace("#type#", getMapping(column.getType()).getType())
    ...
    private String processGetter(Column column) {
        return getLanguageParams()
                .getOrDefault((getStandardType(column.getType()).equals("boolean"))? "#getter_boolean#" : "#getter#", "")
    ...
    private String processSetter(Column column) {
        return getLanguageParams().getOrDefault("#setter#", "")
                .replace("#type#", getMapping(column.getType()).getType())
                .replace("#name#", Format.toCamelCase(column.getName(), false))
                .replace("#Name#", Format.toCamelCase(column.getName(), true));
    }
    ...
    protected String process(String code) {
        return super.process(
                code
                    .replace("#attributes#", processAttributes())
