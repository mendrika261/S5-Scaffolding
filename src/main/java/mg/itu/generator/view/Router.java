...
public class Router extends Scaffolding {
  public Router(HashMap<String, String> params,
  ...
  protected String processDepsRoute() {
    ...
    for (Table table : getDatabase().getTables()) {
      stringBuilder.append(processImport(
          getParams().get("#import_name#").replace("#name#", table.getName()),
          getParams().get("#import_path#").replace("#name#", table.getName())));
    }
  ...
  private String processPaths() {
    ...
    for (Table table : getDatabase().getTables()) {
      paths.append(
          getParams().get("#path#").replace("#name#", table.getName()));
    }
  ...
  protected String process(String code) {
    return super.process(code.replace("#imports#", processDepsRoute())
