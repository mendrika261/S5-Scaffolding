...
public class FormComponentHtml extends Entity {
    public FormComponentHtml(HashMap<String, String> params, HashMap<String, String> languageParams, HashMap<String, Mapping> languageMappings, String packageName, Table table, Provider provider, Database database) {
    ...
    protected String processAttributes() {
        ...
        for (Column column : getTable().getColumns()) {
            ...
            if(column.isForeignKey())
            {
                ...
                options=options .replace("#FKname#",Format.toCamelCase(column.getTableName(),false))
                        .replace("#FKprimaryKey#",Format.toCamelCase(table.getPrimaryKey().getName(),false))
                       .replace("#FKdescription#",Format.toCamelCase(table.getColumns().get(1).getName(),false))
                ;
                ...
                attributeCode.append(
                        attributePart
                                ...
                                .replace("#FKColumnName#",Format.toCamelCase(column.getName(),false))
