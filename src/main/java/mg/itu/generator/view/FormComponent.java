...
public class FormComponent extends Entity {
  ...
  public FormComponent(HashMap<String, String> params,
  ...
  public String processForeignKeys() {
    ...
    for (Column c : list) {
      ...
      foreignKeys.append(
          fk.replace("#FKname#", fkName).replace("#FKtype#", fkType));
      importsDeps.append(processImport(
          fkType,
          getParams().get("interface_path").replace("#FKname#", fkName)));
    }
    return foreignKeys.toString();
  }

  private String processDeps(String deps, String fkType, String fkName) {
    return processImport(fkType,
                         getParams().get(deps).replace("#FKname#", fkName));
  }
  ...
  public String processForeignKeyServices() {
    ...
    for (Column c : list) {
      ...
      foreignKeys.append(
          fk.replace("#FKname#", fkName).replace("#FKtype#", fkType));
      importsDeps.append(processDeps(
          "service_path", fkType + getParams().get("service_suffix"), fkName));
    }
    return foreignKeys.toString();
  }
  ...
  protected String process(String code) {
    return super.process(
        code.replace("#FKtables#", processForeignKeys())
