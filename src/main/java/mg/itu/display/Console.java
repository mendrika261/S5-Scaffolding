...
public class Console {
  ...
  public static void main(String[] args) {
    ...
    final HashMap<String, HashMap<String, String>> components =
        config.getGenerations().get(app);

    // eg: entity, repository ...
    final String component = askToGenerateType(components, scanner);

    final String packageName =
        askString("📂 Enter package name (eg: mg.itu): ", scanner);
  ...
  private static Database askForDatabase(Config config, Scanner scanner) {
    final Database database =
        askChoice("📦 Choose a database: ", config.getDatabases(), scanner);
  ...
  private static String askToGenerate(Config config, Scanner scanner) {
    return askChoiceKey("🛠️ What to generate?", config.getGenerations(),
                        scanner);
  }

  private static String
  askToGenerateType(HashMap<String, HashMap<String, String>> generation,
                    Scanner scanner) {
    HashMap<String, HashMap<String, String>> generationMap =
  ...
  private static String askPath(Scanner scanner, String packageName) {
    ...
    return Format.toPathNameWithTrail(askString(
               "🗃️ Enter the path to save the file(s) (eg: entity/): ",
               scanner)) +
        packageNamePath;
  }

  private static void generate(Config config, HashMap<String, String> params,
    ...
    try {
      ...
      scaffolding =
          (Scaffolding)scaffoldingClass
              .getDeclaredConstructor(HashMap.class, HashMap.class,
                                      HashMap.class, String.class, Table.class,
                                      Provider.class, Database.class)
              .newInstance(
                  params, config.getLanguages().get(params.get("language")),
                  config.getMappings().get(params.get("language")), packageName,
                  table, config.getProviders().get(database.getProvider()),
                  database);
  ...
  private static void generate(Config config, String app, String component,
                               String packageName, List<Table> table,
                               Database database, String path) {
    for (Table t : table) {
      generate(config, config.getGenerations().get(app).get(component),
               packageName, t, database, path);
    }
  }

  private static void generateAll(Config config, String app, String packageName,
                                  List<Table> tables, Database database,
                                  String path) {
    System.out.println();
    System.out.println(COLOR_YELLOW + "Generating all components..." +
                       COLOR_RESET);
    for (String component : config.getGenerations().get(app).keySet()) {
      System.out.println("✅ " + component);
      generate(config, app, component, packageName, tables, database, path);
    }
    System.out.println(COLOR_GREEN + "All components generated successfully!" +
                       COLOR_RESET);
  }
  ...
  private static <T> T askChoice(String question, HashMap<String, T> choices,
                                 Scanner scanner) {
    printQuestion(question);
    ...
    final int choiceIndex =
        askInt("Enter the index of your choice: ", 0, maxChoice, scanner);

    return choices.get(keys[choiceIndex]);
  }

  private static String
  askChoiceKey(String question, HashMap<String, ?> choices, Scanner scanner) {
    printQuestion(question);
  ...
  private static int[] askMultipleAnswers(String question, int min,
                                          int maxChoice, Scanner scanner) {
    ...
    for (int i = 0; i < answers.length; i++) {
      try {
        ...
        if (choices[i] < min || choices[i] > maxChoice) {
          printError("Please enter a number between " + min + " and " +
                     maxChoice);
          return askMultipleAnswers(question, min, maxChoice, scanner);
        }
      } catch (NumberFormatException e) {
        printError(
            "Please enter a number (separate multiple choices with a comma)");
        return askMultipleAnswers(question, min, maxChoice, scanner);
      }
    }
    return choices;
  ...
  askChoices(String question, TreeMap<String, T> choices, Scanner scanner) {
    printQuestion(question);
    ...
    final int maxChoice = choices.size() - 1;
    final int[] choiceIndex = askMultipleAnswers(
        "Enter your choices (separate with a comma):", 0, maxChoice, scanner);
