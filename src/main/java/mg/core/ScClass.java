...
public abstract class ScClass {
    ...
    public ScClass(Table table, Database database) {
        ...
        for (val column : table.getColumns()) {
            val map = Mapping.getMappedType(column.getType(), database.getDbData().getMappings());
    ...
    public String attributesToString() {
        ...
        for (val scAttributes: getAttributes())
            attributes.append("\t").append(scAttributes.getType().getSimpleName()).append(" ").append(scAttributes.getName()).append(";\n");
        return attributes.toString();
    }

    public String convert(String templateConversion) {
        var template = Utils.readFile(Utils.DATA_TEMPLATES_PATH + getLangData().getTemplate(templateConversion));

        for (val toTranslate: getLangData().getTraductions().keySet()) {
            template = template.replace("#package#", getPackageName() == null ? "" : "$package "+getPackageName()+";");
            ...
            template = template.replace("$"+toTranslate, getLangData().getTraduction(toTranslate));
        }
    ...
    public void generate(String path, String templateConversion) {
        ...
        if(!path.isEmpty() && !path.endsWith("/"))
            path += "/";
        Utils.writeFile(path+getNameCamelCase()+getLangData().getExtension(), convert(templateConversion));
    }
}
